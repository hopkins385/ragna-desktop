...
onMounted(async () => {
  ...
  downloadProgressListenerRef = window.electron.ipcRenderer.on('download-progress', (_, payload) => onDownloadProgress(payload))
  downloadCompleteListenerRef = window.electron.ipcRenderer.on('download-completed', (_, payload) => onDownloadCompleted(payload))
})
...
<template>
  <LayoutDefault class="overflow-y-scroll bg-white">
    <div class="mt-5 flex h-44 shrink-0 items-center justify-center rounded-xl border" :style="backgroundImage2">
      <div class="w-96">
        <input v-model="search" class="mx-auto w-full rounded-lg bg-white p-2 text-sm outline-none" type="text"
          placeholder="Search" />
      </div>
    </div>
    <div v-if="results.length > 0" class="my-10 space-y-5 px-20">
      <LlmModelCard v-for="result in results" :key="result.item?.id" :model-id="result.item?.id"
        :title="result.item?.title" :details="result.item?.details" :hf-params="result.item?.hf_params"
        :background-image="backgroundImage" @download="(id) => onDownloadModel(id)"
