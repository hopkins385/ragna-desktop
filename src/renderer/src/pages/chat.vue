...
const submitLocked = computed(() => {
  return !userInput.value || userInput.value.trim() === '' || isStreaming.value === true || model.isModelLoadingInProgress === true || model.isModelLoaded === false
})
...
useMutationObserver(chatMessagesContainerRef,
  (mutations) => {
...
useEventListener(chatMessagesContainerRef, 'wheel', () => {
  ...
  <LayoutDefault>
    <div class="mt-5 flex w-full items-center border-0">
      ...
      <div class="mx-5 flex w-full items-center space-x-5 text-slate-600">
        ...
        <div class="grow">
          <LLMSelector v-model="model.modelPath" :disabled="model.isModelLoaded || model.isModelLoadingInProgress" />
        </div>

        <Button v-if="!model.isModelLoaded && !model.isModelLoadingInProgress" variant="outline"
          :disabled="loadModelLocked" class="shrink-0" @click="() => onLoadModel(model.modelPath)">
          <Icon name="play" class="stroke-2 mr-2 size-4" />
          Load
        </Button>
        <Button v-else-if="model.isModelLoadingInProgress" variant="outline" class="shrink-0" @click="onStop">
        ...
        <Button v-else variant="outline" class="shrink-0" @click="onEjectModel">
          <Icon name="eject" class="stroke-2 mr-2 size-4 -rotate-90" />
          Eject
        </Button>
      </div>
    ...
    <!-- Reset Chat -->
    <div v-if="chat.hasMessages" class="absolute top-20 right-10 z-10 flex border-0 p-4">
      <Button class="group shrink-0 cursor-pointer" variant="ghost" size="icon" @click="() => newChatSession()">
        <Icon name="newChat" class="stroke-1.5 text-primary/30 group-hover:text-primary/100 size-5" />
      </Button>

    </div>
    <!-- Chat Messages Container -->
    <div id="chatMessagesContainer" ref="chatMessagesContainerRef"
      ...
      <!-- Examples -->
      <ChatExamples v-if="showExampleInputs" class="absolute bottom-10 left-0 z-10 w-full"
    ...
    <div class="flex h-20 shrink-0 items-center justify-center">
      <form class="relative h-fit w-full" @submit.prevent="onSubmitUserInput">
        ...
        <div v-if="!isStreaming" class="absolute right-2 top-1/2 cursor-pointer rounded-lg p-2 hover:bg-slate-100"
          style="transform: translateY(-50%)" @click="onSubmitUserInput">
          <Icon name="send" class="stroke-1.5 size-4" :class="{
            'text-primary/30': submitLocked,
            'text-primary': !submitLocked
          }" />
        </div>
        <div v-else class="absolute right-0 top-1/2 cursor-pointer rounded-lg p-2" style="transform: translateY(-50%)"
