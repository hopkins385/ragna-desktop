...
export default function useChat() {
  ...
  async function initChatMessages() {
    ...
    try {
      ...
      if (!resChat.messages || resChat.messages.length < 1) return
  ...
  async function onSubmitUserInput() {
    ...
    try {
      const response = await window.electron.ipcRenderer.invoke('request-llm', {
  ...
  function registerIsStreamingListener() {
    isStreamingListenerRef = window.electron.ipcRenderer.on('llm-is-streaming', isStreamingListener)
  }
