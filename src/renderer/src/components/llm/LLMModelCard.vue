  ...
  <div class="overflow-hidden rounded-xl border bg-white">
    <div class="relative h-52" :style="backgroundImage">
      <div class="absolute inset-0 h-10 space-y-3 p-4">
        ...
        <div class="flex justify-end space-x-2">
          <span v-for="(lang, index) in details.languages" :key="index"
    ...
    <div class="relative flex min-h-20 items-center justify-between p-5">
      <div>
        ...
        <a :href="details.repo_link" target="_blank"
      ...
      <div v-else class="flex flex-col items-center">
        <div class="flex items-center">
          <Button v-if="downloads.isDownloadPending(modelId)" variant="outline" size="icon" class="mr-2"
            @click="$emit('stop', modelId)">
            <SquareIcon class="size-4 fill-slate-800 stroke-none" />
          </Button>
          <Button variant="outline" :disabled="downloads.isDownloadPending(modelId)"
            @click="$emit('download', modelId)">
            {{
              downloads.isDownloadPending(modelId)
                ? Number(downloads.getProgressById(modelId)) < 100 ? `In Progress (${downloads.getProgressById(modelId)}%)`
                  : 'Done' : 'Download' }} </Button>
        </div>
        <div v-if="downloads.isDownloadPending(modelId)" class="text-xxs pt-5 opacity-60">Speed: {{
          downloads.getDlSpeedById(modelId) }} MB/s</div>

      </div>
      <div class="absolute bottom-0 left-0 w-full">
        <div v-if="downloads.getProgressById(modelId)" class="h-2 bg-blue-500"
          :style="{ width: downloads.getProgressById(modelId) + '%' }"></div>
      </div>
    </div>
  </div>
</template>
